...
class EyeEdge(boxes.edges.FingerHoleEdge):
    ...
    def __init__(
        self, boxes, servo, fingerHoles=None, driven=False, outset=False, **kw
    ) -> None:
        self.servo = servo
        self.outset = outset
        self.driven = driven
    ...
    def margin(self) -> float:
        return (
            self.servo.hinge_depth()
            + self.fingerHoles.settings.thickness
            + self.servo.servo_axle * 2
        )
...
class ServoArg:
    ...
    def html(self, name, default, translate):
        options = "\n".join(
            ...
            % (name, ' selected="selected"' if name == default else "", name)
