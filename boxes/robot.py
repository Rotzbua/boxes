    ...
    def html(self, name, default, translate):
        options = "\n".join(
            (
                ...
                % (name, ' selected="selected"' if name == default else "", name, descr)
                for name, descr in self.robotarms
            )
        )
...
class RobotArmMM(_RobotArm):
    ...
    def __call__(self, length, move=None):
        ...
        self.rectangularWall(w, l, "FFFF", move="right")
        self.rectangularWall(
            w, w, "ffff", callback=[lambda: self.servo.front(w / 2)], move="up"
        )
        self.rectangularWall(
            w, w, "ffff", callback=[lambda: self.servo.front(w / 2)], move=""
        )
...
class RobotArmMm(_RobotArm):
    ...
    def __call__(self, length, move=None):
        ...
        self.rectangularWall(
            w2, l, "FfFf", callback=[lambda: self.servo.top(w2 / 2)], move="right"
        )
        self.rectangularWall(
            w2, l, "FfFf", callback=[lambda: self.servo.bottom(w2 / 2)], move="right"
        )
        self.rectangularWall(
            w,
            l,
            "FFFF",
            callback=[None, None, lambda: self.servo2.top(w / 2)],
            move="right",
        )
        self.rectangularWall(
            w,
            l,
            "FFFF",
            callback=[None, None, lambda: self.servo2.bottom(w / 2)],
            move="right",
        )
        self.rectangularWall(
            w2, w, "ffff", callback=[lambda: self.servo.front(w2 / 2)], move="up"
        )
        self.rectangularWall(
            w, w2, "ffff", callback=[lambda: self.servo2.front(w / 2)], move=""
        )
...
class RobotArmUU(_RobotArm):
    ...
    def __call__(self, length, move=None):
        ...
        th = max(2 * self.servo._edges["m"].spacing() + l, 2 * w + 4 * t + self.spacing)
        tw = 5 * (w + 2 * self.thickness + self.spacing)
        ...
        self.rectangularWall(w, l, "FfFf", move="right")
        self.rectangularWall(
            w, w, "ffff", callback=[lambda: self.hole(w / 2, w / 2, 6)], move="up"
        )
        self.rectangularWall(
            w, w, "ffff", callback=[lambda: self.hole(w / 2, w / 2, 6)], move=""
        )
...
class RobotArmUu(_RobotArm):
    ...
    def __call__(self, length, move=None):
        ...
        th = max(
            self.thickness + self.servo._edges["m"].spacing() + l,
        ...
        self.rectangularWall(w, l, e("FFMF"), ignore_widths=iw, move="right")
        self.rectangularWall(
            w2, w, "ffff", callback=[lambda: self.hole(w2 / 2, w / 2, 6)], move="up"
        )
        self.rectangularWall(
            w2, w, "ffff", callback=[lambda: self.hole(w2 / 2, w / 2, 6)], move=""
        )
...
class RobotArmMu(_RobotArm):
    ...
    def __call__(self, length, move=None):
        ...
        th = max(
            t + l + self.servo2._edges["m"].spacing(),
