...
class Planetary2(Boxes):
    ...
    def render(self):
        ...
        t = self.thickness
        spoke_width = 4 * t
        pinsize = self.pinsize / 2.0

        pitch1, size1, xxx = self.gears.sizes(
            teeth=self.sunteeth, dimension=self.modulus
        )
        pitch2, size2, xxx = self.gears.sizes(
            teeth=self.planetteeth, dimension=self.modulus
        )
        ...
        planets = int(
            ...
            / (
                math.asin(
                    float(self.planetteeth + 2) / (self.planetteeth + self.sunteeth)
                )
            )
        )
        ...
        secondary_offsets = [
            (
                (pos % (360.0 / (ringteeth - self.deltateeth)))
                - (pos % (360.0 / ringteeth)) * ringteeth / self.planetteeth
            )
        ...
        self.rectangularTriangle(
            ...
            tl,
            num=8,
            callback=[None, lambda: self.hole(2 * t, 2 * t, screw)],
            move="up",
        )
        ...
        def ring():
            self.gears(
                teeth=ringteeth - self.deltateeth,
            ...
            with self.saved_context():
                ...
                for j in range(2):
                    self.gears(
                        ...
                        dimension=self.modulus,
                        angle=pressure_angle,
                        callback=lambda: self.pins(
                            0.25 * size2, pinsize, i, secondary_offsets[i]
                        ),
                        profile_shift=profile_shift,
                        move="right",
                    )
