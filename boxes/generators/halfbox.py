...
"""
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            ...
            type=float,
            default=25.0,
            help="diameter of clamping holes",
        )
        self.argparser.add_argument(
            "--Mounting",
            action="store",
        ...
        self.argparser.add_argument(
            ...
            type=boolarg,
            default=False,
            help="create sturdy construction (e.g. shelf, clamping jig, ...)",
        )
    ...
    def render(self):
        ...
        if self.Sturdy:
            ...
            self.rectangularWall(
                ...
                width,
                "fffe",
                callback=[None, self.xHoles, None, None],
                move="right",
                label="bottom",
            )
            self.rectangularWall(
                h,
                width,
                "fGfF" if self.Mounting else "fefF",
                callback=[None, None, None, self.hHoles],
                move="up",
                label="back",
            )
            self.rectangularWall(
            ...
            for i in range(2):
                self.move(
                    x + x2 + 2 * t + self.edges["f"].margin(),
                    h + h1 + 2 * t + self.edges["f"].margin(),
                    "right",
                    True,
                    label="side " + str(i),
                )
            ...
            if len(self.sy) > 1:
                for i in range(len(self.sy) - 1):
                    self.move(
                        x + self.edges["f"].margin(),
                        h + self.edges["f"].margin(),
                        "right",
                        True,
                        label="support " + str(i),
                    )
                    self.polygonWallExt(
                        borders=[x, 90, 0, "f", h, 180 - b, 0, "f", l, 180 - c, 0, "e"]
                    )
                    if self.Clamping:
                        self.rectangularHole(
                            x / 2, d / 2 - t / 2, dx=d, dy=d + t, r=d / 8
                        )
                        self.rectangularHole(
                            x - d / 2 + t / 2, h / 2, dx=d + t, dy=d, r=d / 8
                        )
                    self.move(
                        x + self.edges["f"].margin(),
                        h + self.edges["f"].margin(),
            ...
            self.rectangularWall(
                h,
                width,
                "eGeF" if self.Mounting else "eeeF",
                callback=[None, None, None, self.hHoles],
                move="up",
                label="side",
            )
