...
class PirateChest(Boxes):
    ...
    def render(self):
        ...
        if n < 3:
            raise ValueError(
                "number of sides on the lid must be greater or equal to 3 (got %i)" % n
            )
        ...
        fingerJointSettings.setValues(self.thickness, angle=180.0 / (n - 1))
        ...
        with self.saved_context():
            self.rectangularWall(
                x, h, "fFQF", ignore_widths=[2, 5], move="right", label="front"
            )
            self.rectangularWall(
                y, h, "ffof", ignore_widths=[5], move="right", label="right"
            )
        ...
        with self.saved_context():
            self.rectangularWall(
                x, h + hy, e_back, move="right", label="back"
            )  # extend back to correct height
            self.rectangularWall(0, h, "ePee", move="right only")
            self.rectangularWall(
                y, h, "ffOf", ignore_widths=[2], move="right", label="left"
            )
        self.rectangularWall(x, h, "fFOF", move="up only")
        self.rectangularWall(x, 0, "peee", move="up only")
    ...
    def topside(self, y, n, bottem, move=None, label=""):
        ...
        ty = (
            lidheight + self.edges.get("f").spacing() + self.edges.get(bottem).spacing()
        )
