...
![opened spool](static/samples/FilamentSpool-2.jpg)"""
    ...
    def outerHolesCB(self):
        ...
        for i in range(6):
            for j in range(2):
                self.rectangularHole(
                    0,
                    self.outer_diameter / 2 - 7.0,
                    self.outer_diameter * math.pi / 360 * 8,
                    5,
                    r=2.5,
                )
                self.moveTo(0, 0, 10)
        ...
        self.rectangularHole(
            (self.outer_diameter + self.inner_diameter) / 4,
            0,
            (self.outer_diameter - self.inner_diameter) / 2 - 4 * t,
            t,
            r=t / 2,
        )
    ...
    def render(self):
        ...
        self.parts.disc(self.outer_diameter, callback=self.leftsideCB, move="right")
        self.parts.disc(
            self.outer_diameter,
            hole=self.axle_diameter,
            callback=lambda: (self.alignmentHoles(True), self.outerHolesCB()),
            move="right",
        )
