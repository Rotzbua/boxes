...
class MotorEdge(edges.BaseEdge):
    ...
    def __call__(self, l, **kw):
        self.polyline(l - 165, 45, 25 * 2**0.5, -45, 60, -45, 25 * 2**0.5, 45, 55)
...
class Rotary(Boxes):
    ...
    def mainPlate(self):
        ...
        self.hole(1.0 * d, hh, a / 2.0)
        # self.hole(1.0 * d, hh, d/2.)
        self.hole(2.0 * d + 5, hh, a / 2.0)
    ...
    def link(self, x, y, a, middleHole=False, move=None):
        ...
        self.moveTo(y / 2.0, 0)
        self.hole(0, y / 2.0, ra)
        self.hole(x, y / 2.0, ra)

        if middleHole:
            self.hole(x / 2.0, y / 2.0, ra)

        self.edge(10)
        self.edges["F"](60)
        self.polyline(x - 70, (180, y / 2.0), x, (180, y / 2.0))
    ...
    def holderTopCB(self):
        ...
        # XXX
        y = -0.5 * self.diameter + self.th + self.hh - self.beamheight - 2.0
    ...
    def render(self):
        ...
        self.edges["f"].settings.setValues(
            self.thickness, space=2, finger=2, surroundingspaces=1
        )
        ...
        self.rectangularWall(
            ...
            callback=[
                self.holderBaseCB,
                None,
                lambda: self.rectangularHole(hl / 2 + 50, hh - t / 2 - 1, 60, t + 2),
            ],
            move="up",
        )
        self.rectangularWall(
            hl, hh, edges="hfef", callback=[self.holderBaseCB], move="up"
        )
        self.rectangularWall(
            hl,
            hw,
            edges="ffff",
            callback=[lambda: self.hole(hl / 2 - 16 - 20, 25, 5)],
            move="up",
        )
        ...
        with self.saved_context():
            self.rectangularWall(
                ...
                hh,
                edges="hFeF",
                callback=[lambda: self.hole(hw / 2, hh - 20, 4)],
                move="right",
            )
        ...
        self.rectangularWall(
            ...
            edges=[roller2, "f", "e", "f"],
            callback=[
                lambda: self.hole(20, 15, a / 2),
                None,
                lambda: self.rectangularHole(50, th - 15, 70, a, r=a / 2),
            ],
            move="up",
        )
        self.rectangularWall(
            ...
            callback=[
                lambda: self.hole(20, 15, a / 2),
                None,
                lambda: self.rectangularHole(50, th - 15 - t, 70, a, r=a / 2),
            ],
            move="up",
        )
        self.rectangularWall(
            hl,
            th,
            edges=[roller2, "f", RollerEdge(self, None), "f"],
            callback=[self.holderTopCB],
            move="up",
        )
        ...
        with self.saved_context():
            ...
            self.rectangularWall(
                ...
                30,
                edges=["e", "e", slot, "e"],
                callback=[lambda: self.hole(7, 23, self.axle / 2)],
                move="right",
            )
            self.rectangularWall(
                30,
            ...
            self.rectangularWall(
                ...
                30,
                edges=[slot, "e", "e", "e"],
                callback=[lambda: self.hole((hw - 4 * t - 6) / 2.0, 15, 4)],
                move="right",
            )
            for i in range(3):
                self.rectangularWall(
                    20, 30, callback=[lambda: self.nutHole("M8", 10, 15)], move="right"
                )
                self.rectangularWall(
                    20, 30, callback=[lambda: self.hole(10, 15, 4)], move="right"
                )
        ...
        if self.knifethickness:
            ...
            self.rectangularWall(
                ...
                h,
                edges="hfFf",
                callback=[lambda: self.rectangularHole(1.8 * d, 0.5 * bh, bw, bh)],
                move="up",
            )
            self.rectangularWall(
                3.6 * d,
        ...
        self.rectangularWall(
            3.6 * d,
            h,
            edges=["h", "f", MotorEdge(self, None), "f"],
            callback=[self.mainPlate],
            move="up",
        )
        self.rectangularWall(
            3.6 * d,
        ...
        for i in range(3):
            self.parts.disc(
                self.diameter - 2 * self.rubberthickness, hole=self.axle, move="right"
            )
        self.parts.disc(
            self.diameter - 2 * self.rubberthickness, hole=self.axle, move="up right"
        )

        for i in range(3):
            self.parts.disc(
                self.diameter - 2 * self.rubberthickness, hole=self.axle, move="left"
            )
        self.parts.disc(
            self.diameter - 2 * self.rubberthickness, hole=self.axle, move="left up"
        )

        for i in range(3):
            self.parts.disc(
                self.diameter - 2 * self.rubberthickness + 4,
                hole=self.axle,
                move="right",
            )
        self.parts.disc(
