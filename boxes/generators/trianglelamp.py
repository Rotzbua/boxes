...
class TriangleLamp(Boxes):
    ...
    def __init__(self) -> None:
        ...
        self.addSettingsArgs(
            edges.FingerJointSettings, finger=3.0, space=3.0, surroundingspaces=0.5
        )
    ...
    def render(self):
        ...
        self.rectangularTriangle(
            ...
            move="up",
            callback=[
                lambda: self.hole(2 / 3.0 * c, 1 / 4.0 * c, r2),
                lambda: (
                    self.hole(1 / 3.0 * c, 1 / 3.0 * c, r2),
                    self.hole(x - 2 / 3.0 * c, 1 / 4.0 * c, r2),
                ),
            ],
        )
        ...
        self.rectangularWall(x, h, "Ffe" + C, callback=[self.CB(x, c)], move="up")
        self.rectangularWall(x, h, "Ffe" + C, callback=[self.CB(x, c)], move="up")

        self.rectangularWall(x, h, "F" + C + "eF", callback=[self.CB(x, c)], move="up")
        self.rectangularWall(x, h, "F" + C + "eF", callback=[self.CB(x, c)], move="up")

        self.rectangularWall(
            l, h, "F" + C + "e" + C, callback=[self.CB(l, c * 2**0.5)], move="up"
        )
        self.rectangularWall(
            l, h, "F" + C + "e" + C, callback=[self.CB(l, c * 2**0.5)], move="up"
        )
        ...
        self.rectangularTriangle(
            ...
            num=2,
            move="right",
            callback=[lambda: self.hole(2 / 3.0 * c, 1 / 3.0 * c, r1)],
        )
        self.rectangularTriangle(
            c,
            c,
            ...
            num=4,
            move="up",
            callback=[lambda: self.hole(2 / 3.0 * c, 1 / 4.0 * c, r1)],
        )
