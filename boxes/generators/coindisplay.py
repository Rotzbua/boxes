...
class CoinDisplay(Boxes):
    ...
    def bottomHoles(self):
        ...
        """
        self.fingerHolesAt(
            self.x / 2 - self.thickness - self.thickness / 2 - (self.coin_plate / 2),
            self.y / 2 + self.coin_plate_x / 2 - self.thickness,
            self.coin_plate_x,
            -90,
        )
        self.fingerHolesAt(
        ...
        self.fingerHolesAt(
            self.x / 2 - self.coin_plate / 2 - self.thickness,
            self.y / 2 - self.coin_plate_x / 2 - self.thickness * 1.5,
            self.coin_plate,
            0,
        )
    ...
    def render(self):
        ...
        self.rectangularWall(
            ...
            move="right",
            label="Bottom",
            callback=[self.bottomHoles],
        )

        # Draw the coin holder side holsers
        e = ["f", "f", "B", "e"]
        self.rectangularWall(
            self.coin_plate_x, self.coin_showcase_h, e, move="right", label="CoinSide1"
        )
        self.rectangularWall(
            self.coin_plate_x, self.coin_showcase_h, e, move="right", label="CoinSide2"
        )
        ...
        self.rectangularWall(
            ...
            move="down",
            label="Coin Plate",
            callback=[self.coinCutout],
        )

        self.rectangularWall(
            self.coin_plate,
            self.coin_showcase_h,
            "fFeF",
            move="down",
            label="CoinSide3",
        )
