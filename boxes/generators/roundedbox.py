...
"""
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--edge_style",
            action="store",
            type=boxes.ArgparseEdgeType("fFh"),
            choices=list("fFh"),
            default="f",
    ...
    def cb(self, nr):
        ...
        left, l, right = self.surroundingWallPiece(
            nr, self.x, self.y, self.radius, self.wallpieces
        )
        for dh in self.sh[:-1]:
            h += dh
    ...
    def render(self):
        ...
        with self.saved_context():
            self.roundedPlate(
                x, y, r, es, wallpieces=self.wallpieces, extend_corners=ec, move="right"
            )
            for dh in self.sh[:-1]:
                self.roundedPlate(
                    x,
        ...
        self.surroundingWall(
            x, y, r, h, pe, pe, pieces=self.wallpieces, callback=self.cb
        )
