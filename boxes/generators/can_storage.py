...
class FrontEdge(edges.BaseEdge):
    ...
    def __call__(self, length, **kw):
        x = math.ceil(
            (
                (self.canDiameter * 0.5 + 2 * self.thickness)
                * math.sin(math.radians(self.chuteAngle))
            )
            / self.thickness
        )
        if self.top_edge != "e":
        ...
        self.corner(-90, 0.25 * self.canDiameter)
        self.edge(
            0.5 * self.canDiameter
            - (1 + x) * self.thickness
            + self.top_chute_height
            + self.bottom_chute_height
            - self.barrier_height
        )
        self.corner(-90, 0.25 * self.canDiameter)
        self.edge(0.5 * self.canDiameter)
...
"""
    ...
    def __init__(self) -> None:
        ...
        self.addSettingsArgs(
            edges.FingerJointSettings, finger=2.0, space=2.0, surroundingspaces=0.0
        )
        ...
        if nr == 0:
            ...
            if self.fillHoles_fill_pattern != "no fill":
                self.fillHoles(
                    pattern="hbar",
                    border=border,
                    max_radius=min(2 * self.thickness, self.fillHoles_hole_max_radius)
                    if self.fillHoles_fill_pattern in ["hbar", "vbar"]
                    else min(2 * self.thickness, self.width / 30),
                    hspace=min(2 * self.thickness, self.fillHoles_space_between_holes)
                    if self.fillHoles_fill_pattern in ["hbar", "vbar"]
                    else min(2 * self.thickness, self.width / 20),
                    bspace=min(2 * self.thickness, self.fillHoles_space_to_border)
                    if self.fillHoles_fill_pattern in ["hbar", "vbar"]
                    else min(2 * self.thickness, self.width / 20),
    ...
    def cb_bottom_chute(self, nr):
        if nr == 1:
            ...
            self.rectangularHole(
                self.width * 0.85 - 0.5 * self.thickness,
                0.25 * self.pusherA,
                self.thickness,
                0.5 * self.pusherA,
                center_x=False,
            ...
            self.rectangularHole(
                self.width * 0.5 - 0.5 * self.thickness,
                0.25 * self.pusherA,
                self.thickness,
                0.5 * self.pusherA,
                center_x=False,
            ...
            self.rectangularHole(
                self.width * 0.15 - 0.5 * self.thickness,
                0.25 * self.pusherA,
                self.thickness,
                0.5 * self.pusherA,
                center_x=False,
    ...
    def cb_back(self, nr):
        if nr == 1:
            ...
            self.rectangularHole(
                self.width * 0.85 - 0.5 * self.thickness,
                self.thickness
                + self.depth * math.tan(math.radians(self.chuteAngle))
                + 0.25 * self.pusherB,
                self.thickness,
                0.5 * self.pusherB + self.thickness,
                center_x=False,
                center_y=False,
            )
            self.rectangularHole(
    ...
    def cb_sides(self, nr):
        if nr == 0:
            ...
            if self.debug:
                ...
                self.hole(
                    0, self.thickness + self.canDiameter, 1, color=Color.ANNOTATIONS
                )
                self.hole(
                    0,
                    self.thickness + self.canDiameter + self.bottom_chute_height,
                    1,
                ...
                with self.saved_context():
                    ...
                    self.rectangularHole(
                        2 * self.thickness,
                        0,
                        math.ceil(self.canNum / 2) * self.canDiameter,
                        self.canDiameter,
                        center_x=False,
                        center_y=False,
                        color=Color.ANNOTATIONS,
                    ...
                    for i in range(math.ceil(self.canNum / 2) - 1):
                        self.hole(
                            2 * self.thickness + (0.5 + i) * self.canDiameter,
                            self.canDiameter / 2,
                            self.canDiameter / 2,
                            color=Color.ANNOTATIONS,
                        )
                ...
                with self.saved_context():
                    # draw pusher
                    self.moveTo(
                        self.depth - self.pusherA,
                        self.thickness
                        + (self.depth - self.pusherA)
                        * math.tan(math.radians(self.chuteAngle)),
                    )
                ...
                with self.saved_context():
                    ...
                    for i in range(math.ceil(self.canNum / 2)):
                        self.hole(
                            (0.5 + i) * self.canDiameter,
                            self.canDiameter / 2 + 0.5 * self.thickness,
                            self.canDiameter / 2,
                            color=Color.ANNOTATIONS,
                        )
                with self.saved_context():
                    ...
                    self.moveTo(
                        ...
                        1.1 * self.thickness
                        + self.burn
                        + math.sin(math.radians(self.chuteAngle)) * 2 * self.thickness,
                        90,
                    )
                    self.rectangularHole(
                        0,
            ...
            with self.saved_context():
                ...
                self.fingerHolesAt(
                    0, 0, self.depth / math.cos(math.radians(self.chuteAngle)), 0
                )
            # top chute
            with self.saved_context():
                self.moveTo(
            ...
            border = [
                (
                    2 * self.thickness,
                    0.5 * self.thickness
                    + 2 * self.thickness * math.tan(math.radians(self.chuteAngle))
                    + 0.5 * self.thickness / math.cos(math.radians(self.chuteAngle)),
                ),
                (
                    self.depth,
                    self.thickness
                ...
                (
                    self.thickness + 0.75 * self.canDiameter,
                    2 * self.thickness + self.barrier_height,
                ),
    ...
    def render(self):
        ...
        self.pusherB = (
            self.pusherA
            / math.sin(math.radians(180 - (90 + self.chuteAngle) - self.pusherAngle))
            * math.sin(math.radians(self.pusherAngle))
        )
        self.pusherC = (
            self.pusherA
            / math.sin(math.radians(180 - (90 + self.chuteAngle) - self.pusherAngle))
            * math.sin(math.radians(90 + self.chuteAngle))
        )
        ...
        if self.canDiameter < 4 * self.thickness:
            raise ValueError(
                "Can diameter has to be at least 4 times the material thickness!"
            )
        ...
        self.depth = (
            self.canDiameter * (math.ceil(self.canNum / 2) + 0.1) + self.thickness
        )
        self.top_chute_height = max(
            self.depth * math.sin(math.radians(self.chuteAngle)), 0.1 * self.canDiameter
        )
        self.top_chute_depth = (self.depth - 1.1 * self.canDiameter) / math.cos(
            math.radians(self.chuteAngle)
        )
        self.bottom_chute_height = max(
            (self.depth - 1.1 * self.canDiameter)
            * math.sin(math.radians(self.chuteAngle)),
        ...
        if (self.top_chute_depth + self.bottom_chute_height - self.thickness) < (
            ...
            self.bottom_chute_height = (
                self.barrier_height
                + self.canDiameter * 0.1
                + self.thickness
                - self.top_chute_depth
            )
        ...
        self.rectangularWall(
            self.bottom_chute_depth,
            self.width,
            "fefe",
            callback=self.cb_bottom_chute,
            move="up",
            label="bottom chute",
        )
        self.rectangularWall(
            ...
            self.width,
            "fbfe",
            callback=self.cb_top_chute,
            move="up",
            label="top chute",
        )

        self.rectangularWall(
            self.barrier_height, self.width, "fAfe", move="right", label="barrier"
        )
        self.rectangularWall(
            self.height,
            self.width,
            "fefe",
            callback=self.cb_back,
            move="up",
            label="back",
        )
        self.rectangularWall(
            self.barrier_height, self.width, "fefe", move="left only", label="invisible"
        )
        ...
        if self.top_edge != "e":
            self.rectangularWall(
                ...
                self.width,
                "fefe",
                callback=self.cb_top,
                move="up",
                label="top",
            )

        pusherH = (
            self.pusherB * math.cos(math.radians(self.chuteAngle)) + self.thickness
        )
        pusherV = (
            self.pusherC * math.cos(math.radians(self.chuteAngle)) + self.thickness
        )
        ...
        self.DrawPusher()
        self.text(
            "Glue the Pusher pieces into slots on bottom\nand back plates to prevent stuck cans.",
            pusherV + 3,
            0,
            fontsize=4,
            color=Color.ANNOTATIONS,
        ...
        if self.bottom_edge == "š":
            self.rectangularWall(
                self.edges["š"].settings.width + 3 * self.thickness,
                self.edges["š"].settings.height - 4 * self.burn,
                "eeee",
                move="right",
                label="Stabilizer 1",
            )
            ...
            self.rectangularWall(
                self.edges["š"].settings.width + 5 * self.thickness,
                self.edges["š"].settings.height - 4 * self.burn,
                "eeee",
                move="right",
                label="Stabilizer 3",
            ...
            self.text(
                "Glue a stabilizer on the inside of each bottom\nside stacking foot for lateral stabilization.",
                3,
                0,
                fontsize=4,
                color=Color.ANNOTATIONS,
