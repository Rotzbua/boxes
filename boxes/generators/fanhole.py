...
class FanHole(Boxes):
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--arms", action="store", type=int, default=10, help="number of arms"
        )
        self.argparser.add_argument(
            "--inner_disc",
            action="store",
    ...
    def swirl(self, r, ri_rel=0.1, n=20):
        d = 2 * r
        # r = d/2
        ri = ri_rel * r
        ...
        a1 = math.degrees(
            math.atan(
                ri * math.sin(math.radians(ai)) / (r - ri * math.cos(math.radians(ai)))
            )
        )
        d1 = (
            ri * math.sin(math.radians(ai)) ** 2
            + (r - ri * math.cos(math.radians(ai))) ** 2
        ) ** 0.5
        d2 = (
            ri * math.sin(math.radians(ai - ao)) ** 2
            + (r - ri * math.cos(math.radians(ai - ao))) ** 2
        ) ** 0.5
