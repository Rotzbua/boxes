...
"""
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--n", action="store", type=int, default=5, help="number of sides"
        )
        self.argparser.add_argument(
            "--top",
            action="store",
    ...
    def render(self):
        ...
        def drawTop(r, sh, top_type, joint_type):
            if top_type == "closed":
                ...
                self.regularPolygonWall(
                    ...
                    edges=joint_type[1],
                    move="right",
                    callback=[lambda: self.regularPolygonAt(0, 0, n, h=sh - t)],
                )
                if top_type == "angled lid2":
                    self.regularPolygonWall(corners=n, r=r, edges="E", move="right")
            elif top_type in ("hole", "round lid"):
                self.regularPolygonWall(
                    corners=n, r=r, edges=joint_type[1], move="right", hole=(sh - t) * 2
                )
            ...
            if self.top == "bayonet mount":
                ...
                self.regularPolygonWall(
                    corners=n, r=r, edges="F", callback=[self.upperCB], move="right"
                )
                self.parts.disc(sh * 2, move="right")
        ...
        if n % 2:
            ...
            for i in range(n // 2):
                ...
                self.polygonWall(borders1, edge=e1, correct_corners=False, move="right")
