...
class SpicesRack(Boxes):
    ...
    def holes(self):
        ...
        for i in range(self.numx - 1):
            with self.saved_context():
                self.moveTo(
                    (self.diameter + self.space) * (i + 0.5) - (inner_width + t) / 2,
                    self.spacing,
                )
    ...
    def backCB(self):
        ...
        for i in range(self.numy):
            self.fingerHolesAt(
                0, (i + 1) * self.h - 0.5 * self.thickness - dy, self.x, 0
            )
    ...
    def render(self):
        ...
        self.hole_distance = (self.diameter - self.base_r) * math.sin(
        ...
        if self.feet:
            self.partsMatrix(
                self.numx - 1, self.numx - 1, "up", self.foot, width, (self.h - t) / 2
            )
