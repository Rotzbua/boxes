...
class WallPinRow(_WallMountedBox):
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--pins", action="store", type=int, default=8, help="number of pins"
        )
        self.argparser.add_argument(
            "--pinlength",
            action="store",
    ...
    def supportWall(self, move=None):
        ...
        tw = self.edges["a"].margin() + max(
            math.sin(ar) * h / 2 + math.cos(ar) * 5 * t, math.sin(ar) * h
        )
        th = sh + 6
        if self.move(tw, th, move, True):
            return
        ...
        self.polyline(
            math.sin(ar) * 2 * t,
            90 - a,
            math.cos(ar) * 4 * t - math.sin(ar) ** 2 * 2 * t,
            90,
        )
        if a > 0.001:
            self.edges["a"](sh)
