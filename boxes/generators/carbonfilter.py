...
"""
    ...
    def bottomCB(self):
        ...
        for i in range(self.ribs):
            self.fingerHolesAt(
                (i + 1) * self.y / (self.ribs + 1) - 1.5 * t, 0, 4 * t, 90
            )
            self.fingerHolesAt(
                (i + 1) * self.y / (self.ribs + 1) - 1.5 * t, 40 - t, 20, 90
            )

    def topRailCB(self):
        t = self.thickness
        for i in range(self.ribs):
            self.fingerHolesAt((i + 1) * self.y / (self.ribs + 1) - 1.5 * t, 0, 30, 90)
    ...
    def innerRibs(self, n, move=None):
        ...
        for i in range(n):
            ...
            self.edges["f"](30)
            self.polyline(0, 90 + a, 20 - t, 90 - a, l - 20 + t * math.sin(a_), 90 + a)
    ...
    def outerRibs(self, n, n_edge, move=None):
        ...
        for i in range(n):
            ...
            if i < n_edge:
                self.polyline(l - dl2 - t * math.sin(a_), a, dll2, 180 - a, 20)
            else:
                self.polyline(l - dl - t * math.sin(a_), 2 * a, dll, 180 - 2 * a, 20)
            self.edges["f"](30)
    ...
    def render(self):
        ...
        for i in range(self.pockets * 4):
            self.rectangularWall(
                y, 50, "efef", callback=[self.topRailCB], label="Top rails", move="up"
            )
