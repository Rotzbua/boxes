...
class RegularStarBox(Boxes):
    ...
    def render(self):
        ...
        fingerJointSettings.setValues(self.thickness, angle=360.0 / n)
        ...
        for s in range(2):
            with self.saved_context():
                if n % 2:
                    for i in range(n):
                        self.rectangularWall(side, h, move="right", edges="fgeG")
                else:
                    for i in range(n // 2):
                        self.rectangularWall(side, h, move="right", edges="fGeG")
                        self.rectangularWall(side, h, move="right", edges="fgeg")

            self.rectangularWall(side, h, move="up only", edges="fgeG")
