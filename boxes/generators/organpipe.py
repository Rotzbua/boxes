...
class OrganPipe(Boxes):  # Change class name!
    ...
    def __init__(self) -> None:
        ...
        self.addSettingsArgs(
            edges.FingerJointSettings, finger=3.0, space=3.0, surroundingspaces=1.0
        )
...
"""
    ...
    def render(self):
        ...
        speed_of_sound = (
            343.6  # XXX util::speed_of_sound(self.air_temperature); // in m/s
        )
        air_density = 1.2
        ...
        jet_thickness = (f**2 * i**2 * (0.01 * mouth_height) ** 3) / air_speed**2
        sound_power = (
            0.001
            * pi
            ...
            * (
                1.7
                * (jet_thickness * speed_of_sound * f * mouth_area * mouth_area**0.5)
                ** 0.5
            )
            ** 2
        )
        ...
        if self.stopped:
            ...
            resonator_length = (
                -0.73
                * (
                    f * cross_section * 1e-6
                    - 0.342466 * speed_of_sound * mouth_area**0.5 * 1e-3
                )
                / (f * mouth_area**0.5 * 1e-3)
            )
        else:
            ...
            resonator_length = (
                ...
                * (
                    f * cross_section * 1e-6
                    + 0.465753 * f * mouth_area**0.5 * cross_section**0.5 * 1e-6
                    - 0.684932 * speed_of_sound * mouth_area**0.5 * 1e-3
                )
                / (f * mouth_area**0.5 * 1e-3)
            ) * 1e3
        air_hole_diameter = 2.0 * ((mouth_width * jet_thickness * 10.0) ** 0.5 / pi)
        ...
        e = [
            ...
            edges.CompoundEdge(
                ...
                "fef",
                (
                    resonator_length - mouth_height - 10 * t,
                    mouth_height + 10 * t,
                    base_length,
                ),
            ),
        ...
        self.rectangularWall(
            ...
            callback=[
                lambda: self.fingerHolesAt(
                    base_length - 0.5 * t, 0, pipe_depth - jet_thickness
                )
            ],
            move="up",
        )
        ...
        e = [
            edges.CompoundEdge(
                self, "EF", (t * 10, resonator_length - mouth_height - t * 10)
            ),
            "e",
            edges.CompoundEdge(
                self, "FE", (resonator_length - mouth_height - t * 10, t * 10)
            ),
            "e",
        ]
        ...
        self.rectangularWall(
            ...
            "fFfF",
            callback=[
                lambda: self.hole(mouth_width / 2, pipe_depth / 2, d=air_hole_diameter)
            ],
            move="right",
        )
        self.rectangularWall(
            mouth_width, pipe_depth - jet_thickness, "ffef", move="right"
        )
