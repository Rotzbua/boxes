...
class Rack19Box(Boxes):
    ...
    def wallxfCB(self):  # front
        t = self.thickness
        for x in (8.5, self.x + 2 * 17.0 + 2 * t - 8.5):
            for y in (6.0, self.h - 6.0 + t):
                self.rectangularHole(x, y, 10, 6.5, r=3.25)

        self.moveTo(t + 17.0, t)
        self.wallxCB()
    ...
    def _render(self, type):
        ...
        self.rectangularWall(x, h, "fFeF", callback=[self.wallxCB], move="up")
        self.flangedWall(
            x, h, "FFEF", callback=[self.wallxfCB], r=t, flanges=[0.0, 17.0, -t, 17.0]
        )
        self.rectangularWall(y, h, "ffef", callback=[self.wallyCB], move="left up")

        self.rectangularWall(x, y, "fFFF", move="right")
        self.rectangularWall(
            x, y, callback=[lambda: self.hole(trh, trh, d=d2)] * 4, move="up"
        )
