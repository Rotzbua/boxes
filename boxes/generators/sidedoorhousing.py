...
"""
    ...
    def render(self):
        ...
        for move in ("right", "mirror right"):
            re = edges.CompoundEdge(self, ("f", "e"), (bottom.endwidth() + t, h - t))
            if self.double_door:
                le = edges.CompoundEdge(
                    self, ("e", "f"), (h - t, bottom.endwidth() + t)
                )
            else:
                le = "f"
            self.rectangularWall(  # side
                ...
                callback=[
                    ...
                    lambda: (
                        ...
                        self.double_door
                        and self.rectangularHole(
                            y - 1.55 * t, latchpos, 1.1 * t, 1.1 * t
                        ),
                    ),
                ],
                move=move,
        ...
        for i in range(2 if self.double_door else 1):
            self.rectangularWall(
                x, t, (bottom, "F", "e", "F"), ignore_widths=[1, 6], move="up"
            )
            self.rectangularWall(  # back wall
                ...
                "eEeE",
                callback=[
                    lambda: self.fingerHolesAt(0.5 * t, 0, h - 4.05 * t - latchpos),
                    lambda: self.latch_hole(h - 1.2 * t - latchpos),
                    lambda: self.fingerHolesAt(
                        0.5 * t, 3.05 * t + latchpos, h - 4.05 * t - latchpos
                    ),
            ...
            self.rectangularWall(
                x, t, (bottom, "F", "e", "F"), ignore_widths=[1, 6], move="down only"
            )
        if not self.double_door:
            self.rectangularWall(
                x, h, (bottom, "F", "f", "F"), ignore_widths=[1, 6], move="right"
            )
