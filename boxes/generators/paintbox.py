...
class PaintStorage(Boxes):
    ...
    def paintholes(self):
        ...
        if self.hexpattern:
            ...
            self.hexHolesRectangle(
                self.y - 1 * self.minspace, self.x - 1 * self.minspace, settings
            )
            return
        ...
        for i in range(n_y):
            for j in range(n_x):
                self.hole(
                    i * (self.candiameter + spacing_y)
                    + (self.candiameter + spacing_y) / 2,
                    j * (self.candiameter + spacing_x)
                    + (self.candiameter + spacing_x) / 2,
                    self.candiameter / 2,
                )
    ...
    def sidesCb(self):
        ...
        if not self.drawer:
            ...
            if self.additional_top:
                self.fingerHolesAt(
                    self.canheight - ((self.canheight / 6) + t), -t, x, 90
                )
        else:
            self.rectangularHole(h / 3, (x / 2.0) - t, hh, hx, r=hr)
    ...
    def render(self):
        ...
        if not self.drawer:
            ...
            if self.additional_bottom:
                self.rectangularWall(
                    y, x, "efef", callback=[self.paintholes], move="up"
                )
            if self.additional_top:
                self.rectangularWall(
                    y, x, "efef", callback=[self.paintholes], move="up"
