...
class WineRack(Boxes):
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--radius", action="store", type=float, default=46.0, help="Radius of comb"
        )
        self.argparser.add_argument(
            "--walls",
            action="store",
    ...
    def wallCB(self, frontwall=False, backwall=False):
        ...
        for i in range(cy // 2 + cy % 2):
            ...
            for j in range(cx):
                ...
                if self.walls == "all":
                    self.hexFingerHoles(
                        j * 2 * dx + 2 * dx, (2 * r + 2 * dy) * i + dy, r, 90
                    )
                if wmin and i == cy // 2:  # top row
                    continue
                if j > 0 or not wmin:
                    self.hexFingerHoles(
                        j * 2 * dx + dx, (2 * r + 2 * dy) * i + r + 2 * dy, r, -150
                    )
                if j < cx - 1 or not wmin:
                    self.hexFingerHoles(
                        j * 2 * dx + dx, (2 * r + 2 * dy) * i + r + 2 * dy, r, -30
                    )
            if i < cy // 2:
                for j in range(cx):
                    if not frontwall and self.walls == "all":
                        self.hexFingerHoles(
                            j * 2 * dx + dx, (2 * r + 2 * dy) * i + r + 2 * dy, r, 90
                        )
                if not backwall:
                    for j in range(1, cx):
    ...
    def render(self):
        ...
        self.delta = 3**0.5 / 6.0 * t

        self.rectangularWall(x, y, callback=[self.wallCB], move="up")
        self.rectangularWall(
            x, y, callback=[lambda: self.wallCB(backwall=True)], move="up"
        )
        self.rectangularWall(
            x, y, callback=[lambda: self.wallCB(frontwall=True)], move="up"
        )
        if self.walls == "all":
            tc = (cy // 2 + cy % 2) * (6 * cx + 1)
        else:
        ...
        self.partsMatrix(
            tc, cx, "up", self.rectangularWall, r - 2 * self.delta, h, "fefe"
        )
