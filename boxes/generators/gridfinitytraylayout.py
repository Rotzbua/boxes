...
"""
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            ...
            type=int,
            default=3,
            help="number of gridfinity grids in X direction",
        )
        self.argparser.add_argument(
            "--ny",
            type=int,
            default=2,
            help="number of gridfinity grids in Y direction",
        )
        self.argparser.add_argument(
            "--countx",
            type=int,
            default=5,
            help="split x into this many grid sections.  0 means same as --nx",
        )
        self.argparser.add_argument(
            "--county",
            type=int,
            default=3,
            help="split y into this many grid sections.  0 means same as --ny",
        )
        self.argparser.add_argument(
            "--margin",
            type=float,
        ...
        self.argparser.add_argument(
            "--layout",
            type=str,
            help="You can hand edit this before generating",
            default="",
        )
        ...
        with self.saved_context():
            for xx in [0, self.nx - 1]:
                for yy in [0, self.ny - 1]:
                    ...
                    self.rectangularEtching(
                        x + p / 2 + xx * p, y + p / 2 + yy * p, o - m, o - m
                    )
            self.ctx.stroke()
    ...
    def render(self):
        ...
        self.walls()
        with self.saved_context():
            self.base_plate(callback=[self.baseplate_etching], move="mirror right")
            foot = self.opening - self.opening_margin
            for i in range(min(self.nx * self.ny, 4)):
                self.rectangularWall(foot, foot, move="right")
        self.base_plate(callback=[self.baseplate_etching], move="up only")
        self.lid(
