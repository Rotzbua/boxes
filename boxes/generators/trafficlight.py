...
When turned by 90°, it can be also used to create a bottle holder."""
    ...
    def __init__(self) -> None:
        ...
        self.argparser.add_argument(
            "--n", action="store", type=int, default=3, help="number of lights"
        )
        self.argparser.add_argument(
            "--upright",
            action="store",
    ...
    def addMountH(self, width, height):
        ...
        if len(self.hole_dD) < 2:  # if no head diameter is given
            ...
            y = height - max(
                self.thickness * 1.25, self.thickness * 1.0 + ds
            )  # and we assume that a typical screw head diameter is twice the shaft diameter
        else:
            dh = self.hole_dD[1]  # use given head diameter
            y = height - max(
                self.thickness * 1.25, self.thickness * 1.0 + dh / 2
            )  # and offset the hole to have enough space for the head
    ...
    def addMountV(self, width, height):
        ...
        if len(self.hole_dD) < 2:  # if no head diameter is given
            dh = 0  # only a round hole is generated
            x = max(
                self.thickness * 2.75, self.thickness * 2.25 + ds
            )  # and we assume that a typical screw head diameter is twice the shaft diameter
        else:
            dh = self.hole_dD[1]  # use given head diameter
            x = max(
                self.thickness * 2.75, self.thickness * 2.25 + dh / 2
            )  # and offset the hole to have enough space for the head
    ...
    def render(self):
        ...
        if self.upright:
            self.rectangularWall(
                ...
                h,
                "FFFF",
                callback=[self.backCB, self.addMountV(th, h)],
                move="up",
                label="back",
            )
        else:
            self.rectangularWall(
                ...
                h,
                "FFFF",
                callback=[self.backCB, self.addMountH(th, h)],
                move="up",
                label="back",
            )
        ...
        if self.upright:
            # sides
            self.rectangularWall(
                th, d, "fFsF", callback=[self.sideCB], move="up", label="left"
            )
            self.rectangularWall(
                th, d, "fFsF", callback=[self.sideCB], move="up", label="right"
            )
            ...
            # bottom
            self.rectangularWall(
                th, d, "fFeF", callback=[self.sideCB], move="up", label="bottom"
            )
            # top
            self.rectangularWall(
                th, d + s, "fFeF", callback=[self.topCB], move="up", label="top"
            )
            # vertical walls
            for i in range(n):
                self.wall(
