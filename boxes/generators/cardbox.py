...
"""
    ...
    def render(self):
        ...
        s = FingerHoleEdgeSettings(
            thickness=t, wallheight=h, fingerholedepth=self.fingerholedepth
        )
        ...
        if self.openingdirection == "right":
            with self.saved_context():
                self.rectangularWall(x, y - t * 0.2, "eFee", move="right", label="Lid")
                self.rectangularWall(
                    x,
                    y,
                    "ffff",
                    callback=[self.divider_bottom],
                    move="right",
                    label="Bottom",
                )
            ...
            with self.saved_context():
                self.rectangularWall(
                    y, h + t, "FFEF", move="right", label="Outer Side Left"
                )
                self.rectangularWall(
                    y, h + t, "FFaF", move="right", label="Outer Side Right"
                )
            self.rectangularWall(y, h + t, "fFfF", move="up only")

            with self.saved_context():
                self.rectangularWall(
                    y, h, "Aeee", move="right", label="Inner Side Left"
                )
                self.rectangularWall(
                    y, h, "Aeee", move="right", label="Inner Side Right"
                )
            self.rectangularWall(y, h, "eAee", move="up only")

            with self.saved_context():
                self.rectangularWall(
                    y - t * 0.2, t, "fEeE", move="right", label="Lid Lip"
                )
            ...
            if self.add_lidtopper:
                self.rectangularWall(
                    x, y - 2 * t, "eeee", move="right", label="Lid topper"
                )

        elif self.openingdirection == "front":
            with self.saved_context():
                self.rectangularWall(x - t * 0.2, y, "eeFe", move="right", label="Lid")
                self.rectangularWall(
                    x,
                    y,
                    "ffff",
            ...
            self.rectangularWall(x, y, "eEEE", move="up only")
            self.rectangularWall(x - t * 0.2, t, "fEeE", move="up", label="Lid Lip")
            ...
            with self.saved_context():
                self.rectangularWall(
                    y, h + t, "FfFf", move="right", label="Outer Side Left"
                )
                self.rectangularWall(
                    y, h + t, "FfFf", move="right", label="Outer Side Right"
                )
            self.rectangularWall(y, h + t, "fFfF", move="up only")
            ...
            if self.add_lidtopper:
                self.rectangularWall(
                    x, y - 2 * t, "eeee", move="right", label="Lid topper (optional)"
                )
